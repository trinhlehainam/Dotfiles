...
This branch intentionally moved the Neovim hook from `hooks.apply.pre` to `hooks.read-source-state.pre` and removed the legacy script entry points. Regenerate your active chezmoi config before the next `chezmoi apply`, or the old hook path in your local config will fail.
